...
*  @return:	uint8_t
*  @note:	需要自己配置端口编号
*  @sample:	KeyNum = Key_GetNum(GPIO_B,GPIO_Pin_0);
 **************************************************************/
uint8_t Key_GetNum(GPIO_TypeDef* Peripheral_declaration)
{
	uint8_t KeyNum = 0;
	if(GPIO_ReadInputDataBit(Peripheral_declaration,GPIO_Pin_1) == 0)
	{
		Delay_ms(20);
		while(GPIO_ReadInputDataBit(Peripheral_declaration,GPIO_Pin_1) == 0);
		KeyNum = 1;
	}
	if(GPIO_ReadInputDataBit(Peripheral_declaration,GPIO_Pin_11) == 0)
	{
		Delay_ms(20);
		while(GPIO_ReadInputDataBit(Peripheral_declaration,GPIO_Pin_11) == 0);
		KeyNum = 2;
	}
	return KeyNum;
}
